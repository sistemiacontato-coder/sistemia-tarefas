  ...
  async createTask(taskData: Omit<Task, 'id' | 'created_at' | 'owner_id'>): Promise<Task> {
    ...
    if (isRealSupabaseConfigured && supabaseRealClient) {
      try {
        ...
        // Remove campos de data quando vazios — coluna deve aceitar NULL no Supabase
        if (!dbTaskData.start_date) delete dbTaskData.start_date;
        if (!dbTaskData.end_date) delete dbTaskData.end_date;
  ...
  async updateTask(id: string, updates: Partial<Omit<Task, 'id' | 'created_at'>>): Promise<Task> {
    if (isRealSupabaseConfigured && supabaseRealClient) {
      try {
        ...
        // Remove campos de data vazios do UPDATE
        const safeUpdates: any = { ...updates };
        if (safeUpdates.start_date === '') delete safeUpdates.start_date;
        if (safeUpdates.end_date === '') delete safeUpdates.end_date;
        const { data, error } = await supabaseRealClient
          .from('sia_tarefas_tasks')
          .update(safeUpdates)
          .eq('id', id)
